  ...
  return (
    <nav className="navber bg-gray-900 text-gray-50 h-20 border-b-2 border-yellow-500 px-5 flex justify-between items-center">
      ...
      <div className="right flex gap-5 items-center mr-5">
        <Link
          to="/"
          className="bottom-border hover:text-yellow-500 duration-300"
        >
          Home
        </Link>
        <Link
          to="/products"
          className="bottom-border hover:text-yellow-500 
         duration-300"
        >
          Products
        </Link>
        <Link to="/cart">
          ...
          <BsCart />
          <span className="cart-icon absolute z-[1] top-3 right-7 text-rose-500 bg-yellow-300 h-5 w-5 text-center rounded-full text-sm font-bold">
            {cartItems.length}
          </span>
        </Link>
      </div>
